...
public class randomizer {
    ...
    public randomizer(String inputROM, String outputPath) {
        ...
        GregorianCalendar sec = new GregorianCalendar();
        String suffix = Integer.toString(Math.abs((int)sec.getTimeInMillis() * 1000));
        this.output = new File(outputPath + "/PKseed" + suffix + ".z64").toPath();
    }

    public void Randomize() throws IOException, incorrectROMExtensionException {
		if (rom.toString().endsWith(".z64"))
			Files.copy(rom, output);
		else
			throw new incorrectROMExtensionException("ROM must be a .z64 file.");
        ...
        try {
    		...
    		System.out.println("IOException in Randomize(). Could not write new base stats.");
    		ioe.printStackTrace();
    	}

        // randomize gym castle trainer pokemon
        try {
    		...
    		System.out.println("IOException in Randomize(). Could not randomize gym leader castle pokemon.");
    		ioe.printStackTrace();
    	}
        ...
        try {
    		...
    		System.out.println("IOException in Randomize(). Could not randomize gym leader castle rentals.");
    		ioe.printStackTrace();
    	}
